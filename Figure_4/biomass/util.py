...
def optimize(model, *args, max_generation=100):
    warnings.filterwarnings('ignore')
    ga_init = GeneticAlgorithmInit(
        model,
        max_generation=max_generation, #10000
        allowable_error=0.0,#.32, # 10% percentage error x 32 data = 0.01 x 32
    )
    if len(args) == 1:
...
def optimize_continue(model, *args, max_generation=1000):
    warnings.filterwarnings('ignore')
