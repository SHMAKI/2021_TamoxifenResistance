...
#from .observable import custom_vals #追加
...
class Visualization(object):
    ...
    def __init__(self):
        ...
        #print("conditions\n")
        #print(sim_vis.conditions)
        #print(len(sim_vis.conditions))
        #print([range(0, 20, 2), range(20)][len(sim_vis.conditions) > 10])
        ...
        #         'ylim' : (),
        #         'yticks' : None,
        #         'ylabel': custom_vals[i].replace('__', '\n').replace('_', ' '),
        #         'exp_data' : False,  # if False, experimental data will not be shown
        #         'cmap' : [self.cm.colors[j] for j in cm_range], #range(20)
        #         'shape' : Line2D.filled_markers,
        #         'dont_show' : [],  # conditions you don't want to plot
        #     } for i, _ in enumerate(custom_vals)]
    ...
    def get_timecourse_options(self):
        for i, _ in enumerate(observables):
            ...
            # #####
            # self.timecourse_options[
            #     custom_vals.index('Sensitive_Cells')
            # ]['ylabel'] = 'Sensitive_Cells'
            # self.timecourse_options[
            #     custom_vals.index('Resistant_Cells')
            # ]['ylabel'] = 'Resistant_Cells'
            # #####
